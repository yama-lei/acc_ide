...
class MainActivity : AppCompatActivity() {
    ...
    private fun cleanupExpiredDeletionMarkers() {
        try {
            ...
            if (orphanedMetaFiles.isNotEmpty()) {
                Log.d("MainActivity", "发现 ${orphanedMetaFiles.size} 个孤立的元数据文件，正在清理...")
    ...
    private fun loadFilesFromStorage() {
        ...
        files.clear()
            ...
            if (notExistFiles.isNotEmpty()) {
                ...
                saveOpenedFilesList()
                Log.d("MainActivity", "从已打开文件列表中清理了 ${notExistFiles.size} 个不存在的文件: ${notExistFiles.joinToString()}")
            }
        ...
        val storedFiles = fileStorageManager.getAllFiles()
            ...
            if (openedFiles.isEmpty() && storedFiles.isNotEmpty()) {
                ...
                if (validFiles.isNotEmpty()) {
                    ...
                    saveOpenedFilesList()
                    Log.d("MainActivity", "没有已打开的文件，添加了 ${validFiles.size} 个文件到文件树: ${validFiles.map { it.name }.joinToString()}")
                }
            }
            ...
            Log.d("MainActivity", "从存储加载了 ${openedFiles.size} 个已打开的文件，有效文件: $validFileCount 个")
    ...
    fun createNewFile(language: String) {
        try {
            ...
            if (filesToRemove.isNotEmpty()) {
                Log.d("MainActivity", "从内存中移除不存在于文件系统的文件: ${filesToRemove.joinToString()}")
    ...
    private fun saveCurrentFile() {
        if (currentFileName.isNotEmpty() && files.containsKey(currentFileName)) {
            ...
            Log.d("MainActivity", "已保存当前文件: $currentFileName, 外部保存状态: $isExternallySaved")
        }
    }
    ...
    fun updateFileContent(fileName: String, content: String) {
        if (files.containsKey(fileName)) {
            ...
            Log.d("MainActivity", "已更新文件内容: $fileName, 保存状态: $success, 外部保存: $isExternallySaved")
        }
    }
    ...
    private fun closeFile(fileName: String) {
        try {
            ...
            Log.d("MainActivity", "文件 $fileName 外部保存状态: $isExternallySaved, URI: $externalUri")
            ...
            Toast.makeText(this, getString(R.string.file_closed, fileName), Toast.LENGTH_SHORT).show()
        } catch (e: Exception) {
        ...
        try {
            ...
            val language = getFileLanguage(fileName)
                ...
                if (success) {
                    ...
                    Log.d("MainActivity", "文件已直接更新到外部: $fileName, URI: ${codeFile.externalUri}")
                } else {
    ...
    private fun handleSaveFile(fileName: String, uri: Uri) {
        try {
            ...
            if (fileStorageManager.saveFileToUri(updatedCodeFile, uri)) {
                ...
                // 更新文件列表UI，显示没有星号的文件名
                if (::fileListAdapter.isInitialized) {
                    fileListAdapter.updateFileList(openedFiles.toList(), currentFileName, externalSaveStatus)
                }
